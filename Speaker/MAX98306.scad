...
module max98306_main(target=TARGET){
    ...
    if (target == "default") {
        max98306(with_vent=1);
    }

    if (target == "novent") {
        max98306(with_vent=0);
    }

    if (target == "body_only") {
        max98306(with_body=1, with_lid=0);
    }

    if (target == "body_test") {
        with_vent=1; with_print=0;
        ox = size_x + shell_thick*2;
        oy = size_y + shell_thick*2;
        oz = size_z + shell_thick*2;
        translate([-ox/2,0,0])
        max98306_unit_body_only(ox=ox, oy=oy, oz=oz, with_vent=with_vent, with_cabling=0, with_lid=1, with_shell=0, with_holder=1, with_side=0, with_print=with_print, with_open_roof=1);
    }
    
    if (target == "lid_only") {
        max98306(with_body=0, with_lid=1);
    }

    if (target == "unit_test") {
        translate([40,0,0]){
            max98306_unit(ox=40, oy=38, oz=36, with_vent=0, with_cabling=1, with_lid=0, with_shell=1, with_holder=1, with_side=1);
        }
        max98306_unit_lid_only(ox=40, oy=38, oz=36, with_vent=0, with_cabling=1, with_lid=0, with_shell=0, with_holder=0, with_hole=0, with_vent_lid=1);
    }
    
    if (target == "unit_lid_test") {
        max98306_unit_lid_only(ox=40, oy=38, oz=36, lid_thick=3, with_vent=0, with_cabling=1, with_lid=0, with_shell=0, with_holder=0, with_hole=0, with_vent_lid=1, with_arm=1);
    }
...
module max98306(with_vent=1, with_vent_lid=0, with_lid=1, with_body=1, with_print=1){
    ...
    if (with_body) {
        translate([size_x/2+shell_thick,0,size_x/2])
         {
            rotate([0,90,0]) {
                max98306_unit_body_only(ox=ox, oy=oy, oz=oz, with_vent=with_vent, with_cabling=0, with_lid=1, with_shell=0, with_holder=1, with_side=0, with_print=with_print);
            }
        }
    }
    ...
    if (with_lid) {
            rotate([0,-90,0]) {
            max98306_unit_lid_only(ox=ox, oy=oy, oz=oz, lid_thick=shell_thick, with_vent=with_vent, with_cabling=0, with_lid=1, with_shell=0, with_holder=1, with_side=0, with_vent_lid=with_vent_lid, with_print=with_print);
        }
    }
}
...
// with_holder = { 1: render the holder to support MAX98306 board. 0: without holder (testing purpose). }
// with_side   = { 1: fill in the side space between the shell and the holder (to avoid oozing). 0: does not do that. }
// with_print = { 1: print layout (models are rotated to be printable), 0: edit layout (no rotation) }
module max98306_unit(ox, oy, oz, lid_thick=shell_thick, with_vent, with_cabling, with_lid, with_shell, with_holder, with_side=0, with_vent_lid=0, with_print=1, with_open_roof=0){
    if (with_print){
        rotate([0,90,0]) {
            //translate([-ox/2,oy/2-10,oz/2]) {
            translate([0,0,0]) {
                max98306_unit_body_only(ox=ox, oy=oy, oz=oz, with_vent=with_vent, with_cabling=with_cabling, with_lid=with_lid, with_shell=with_shell, with_holder=with_holder, with_side=with_side, with_open_roof=with_open_roof);
            }
        }
    } else {
        max98306_unit_body_only(ox=ox, oy=oy, oz=oz, with_vent=with_vent, with_cabling=with_cabling, with_lid=with_lid, with_shell=with_shell, with_holder=with_holder, with_side=with_side, with_open_roof=with_open_roof);
    ...
    if (with_lid) {
        translate([-ox/2,0,0]) {
            max98306_unit_lid_only(ox=ox, oy=oy, oz=oz, lid_thick=lid_thick, with_vent=with_vent, with_cabling=with_cabling, with_lid=with_lid, with_shell=with_shell, with_holder=with_holder, with_vent_lid=with_vent_lid);
        }
    }
}
...
// with_side   = { 1: fill in the side space between the shell and the holder (to avoid oozing). 0: does not do that. }
// with_print = { 1: print layout (models are rotated to be printable), 0: edit layout (no rotation) }
module max98306_unit_body_only(ox, oy, oz, with_vent, with_cabling, with_lid, with_shell, with_holder, with_side=0, with_open_roof=0){
    ...
    translate([0,0,0]) {
        if (with_shell) {
            difference(){
                union() {
                    max98306_shell(ox, oy, oz, shell_thick, with_vent, with_cabling);
                }
                // put the negative volume at exactly same position as holder
                translate([-shell_thick, hy, hz-dz+shell_thick]) {
                    rotate([0,0,0]) {
                        #max98306_holder_negative_volume(ox, size_y, size_z, shell_extend, shell_thick, with_vent, with_cabling);
                    }
                }
            }
        }
        if (with_holder) {
            translate([hx, hy, hz-dz]) {
                rotate([0,0,0]) {
                    max98306_holder(size_x, size_y, size_z, ox, oy, oz, shell_extend, shell_thick, with_vent=with_vent, with_cabling=with_cabling, with_side=with_side, with_open_roof=with_open_roof);
                }
            }            
        }
    }
...
// with_side   = { 1: fill in the side space between the shell and the holder (to avoid oozing). 0: does not do that. }
// with_print = { 1: print layout (models are rotated to be printable), 0: edit layout (no rotation) }
module max98306_unit_lid_only(ox, oy, oz, lid_thick=shell_thick, with_vent, with_cabling, with_lid, with_shell, with_holder, with_hole=1, with_vent_lid=0, with_arm=0){
    // put holder at the left-top corner (looking from back side) = right-top corner (looking from front side)
    hx = ox/2 - size_x/2 - shell_thick;
    hy = 0;
    hz = oz/2 - size_z/2 - shell_thick;
...
module max98306_shell(ox, oy, oz, st, with_vent, with_cabling) {
    // outer shell
    difference(){
        difference(){
...
module max98306_lid_holder_extension(sx, sy, sz, sh, st, with_vent, with_cabling) {
    // inner holder
...
// holder to support MAX98306 board
module max98306_holder(sx, sy, sz, ox, oy, oz, sh, st, with_vent, with_cabling, with_side=0, with_open_roof=0) {
        ...
        difference(){
            union(){
                ...
                if (sh > 0) { // extend shell in x-axis to support lid
                    max98306_lid_holder_extension(sx, sy, sz, sh, st, with_vent, with_cabling);
                }
                translate([WELDING,0,0]) {
                    max98306_hold_arm(sx, sy, sz, with_vent);
                }
            }
            union() {
                max98306_holder_negative_volume(sx, sy, sz, sh, st, with_vent=with_vent, with_cabling=with_cabling); // shared with shell
                max98306_holder_negative_volume_private(sx, sy, sz, sh, st, with_vent=with_vent, with_cabling=with_cabling, with_open_roof=with_open_roof); // private to holder
            }
        }
    }
}
...
module max98306_holder_negative_volume(sx, sy, sz, sh, st, with_vent, with_cabling) {
    // front knob hole
...
module max98306_holder_negative_volume_private(sx, sy, sz, sh, st, with_vent, with_cabling, with_open_roof=0) {
    // open roof
    if (with_open_roof) {
    ...
    if (with_vent) {
        // side vent
        for (i=[1]) {
            translate([-st*1.5,0,i*3.5-1]) {
                #prim_round_yplate0(sx*0.8, sy+st*2+WELDING, 2.4, 1.2, 16);
            }
        }
        ...
        // front vent
        for (i=[1]) {
            translate([sx/2+shell_thick,0,i*3.5-1]) {
                #prim_round_xplate0(shell_thick*4, sy*0.9, 2.4, 1.2, 16);
            }
        }
    }
...
module max98306_hold_arm(ox, oy, oz, with_vent){
    // for debug
    //translate([-size_x*1.6,0,0])
    {
        ...
        translate([-arm_thick, 0, -size_z/2+arm_z/2]) {
            difference(){
                ...
                union(){
                    // wide open it
                    translate([-arm_thick*2,0,-WELDING]) {
